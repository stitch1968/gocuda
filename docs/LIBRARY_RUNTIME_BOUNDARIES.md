...
- `libraries/thrust.go`: Thrust-style algorithms are implemented by deterministic Go execution with explicit host/device transfers and are production-ready across simulation and CUDA runtime modes. Native CUDA-tagged Thrust context/copy/fill/generate dispatch is now wired for CUDA builds, with deterministic fallback preserved for the broader algorithm surface.
...
- `libraries/thrust.go`: native CUDA-tagged Thrust parity is now in progress for a bounded runtime-backed context/copy/fill/generate slice; broader algorithm coverage and hardware validation are still pending.
