...
- [ ] Thrust: add native CUDA-tagged implementations to complement the current deterministic production-ready implementation (in progress: native context creation, runtime-backed copy/fill/generate dispatch, CUDA-tagged native tests, and deterministic fallback for the remaining algorithm surface added; pending hardware validation and broader native algorithm coverage)
...
- [ ] nvJPEG2000: add native CUDA-tagged nvJPEG2000 bindings to complement the current `ffmpeg`/`ffprobe`-backed production-ready implementation (in progress: native handle/state lifecycle, native metadata probing, native 8-bit interleaved encode/decode dispatch, CUDA-tagged native tests, and Windows import-lib setup path added; pending hardware validation)
- [ ] CUTLASS: add native CUDA kernels to complement the current deterministic production-ready implementation (in progress: native cuBLAS-backed float32/float64 row-major GEMM dispatch, float32 Rank2k with beta=0, float32 Trmm, CUDA-tagged native tests, and Windows import-lib setup path added; pending hardware validation and broader operation coverage)
