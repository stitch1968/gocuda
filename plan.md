...
- [ ] CUTLASS: add native CUDA kernels to complement the current deterministic production-ready implementation (in progress: native cuBLAS-backed float32/float64 row-major GEMM dispatch, CUDA-tagged native tests, and Windows import-lib setup path added; pending hardware validation and broader operation coverage)
